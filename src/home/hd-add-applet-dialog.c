...
/* Pixel sizes */
#define ADD_APPLET_DIALOG_WIDTH 342
#define ADD_APPLET_DIALOG_HEIGHT 80

#define ADD_APPLET_DIALOG_CLOSE  43
#define ADD_APPLET_DIALOG_ICON  24

#define MARGIN_DEFAULT 8
#define MARGIN_HALF 4

/* Timeout in seconds */
#define ADD_APPLET_DIALOG_PREVIEW_TIMEOUT 4
...
static void
hd_add_applet_dialog_dispose (GObject *object)
{
  HDAddAppletDialogPrivate *priv = HD_ADD_APPLET_DIALOG (object)->priv;
  ...
  G_OBJECT_CLASS (hd_add_applet_dialog_parent_class)->dispose (object);
}

static void
hd_add_applet_dialog_finalize (GObject *object)
{
  /* HDAddAppletDialogPrivate *priv = HD_ADD_APPLET_DIALOG (object)->priv; */

  G_OBJECT_CLASS (hd_add_applet_dialog_parent_class)->finalize (object);
}
...
hd_add_applet_dialog_class_init (HDAddAppletDialogClass *klass)
{
  ...
  object_class->dispose = hd_add_applet_dialog_dispose;
  object_class->finalize = hd_add_applet_dialog_finalize;

  g_type_class_add_private (klass, sizeof (HDAddAppletDialogPrivate));
}

static void
response_cb (HDAddAppletDialog *dialog,
             gint               response_id,
             gpointer           data)
{
  HDAddAppletDialogPrivate *priv = dialog->priv;
  ...
  if (response_id == GTK_RESPONSE_OK)
    {
      ...
      g_debug ("New applet was selected");
...
hd_add_applet_dialog_init (HDAddAppletDialog *dialog)
{
  ...
  /* Set dialog title */
  gtk_window_set_title (GTK_WINDOW (dialog), gettext ("home_ti_select_applets"));

  /* */
  priv->selector = g_object_ref (hildon_touch_selector_new ());

  priv->store = GTK_LIST_STORE (hd_applet_manager_get_model (hd_applet_manager_get ()));

  /* Text column */
  hildon_touch_selector_append_text_column (HILDON_TOUCH_SELECTOR (priv->selector),
                                            hd_applet_manager_get_model (hd_applet_manager_get ()),
                                            TRUE);
